...
- The LLM-assisted producer is primary. Deterministic fallback is eligible only before any rubric state has been declared or observed, and only for `provider-unconfigured`, `provider-unavailable`, `provider-timeout`, `primary-output-malformed`, or `primary-output-structurally-invalid`; every other condition fails closed. A valid active rubric is reused. Declared-but-invalid, duplicate, staging, recovery-required, conflicted, unreadable, unavailable, or mismatched state blocks fallback and remains recovery-blocked.
- On an admitted fallback, emit the fixed Task-Intent Baseline v1 rows in this canonical order: `new-observable-behavior=strict-tdd`, `bug-fix=strict-tdd`, `security-state-data-mutation=strict-tdd`, `migration-schema=strict-tdd`, `behavior-preserving-refactor=standard`, `ui-layout-style-only=standard`, `ui-behavior-or-bug=strict-tdd`, `cross-layer-port-wiring=standard`, `prompt-model-policy=standard`, `config-infra-operations=standard`, `executable-script=standard`, `docs-runbook-metadata=skip`, `dependency-bump=standard`, and unmatched-only `default=standard`. Bind only detected satisfiable project capabilities; do not infer project-specific rows, commands, evidence, or local rules. Record `baseline_version: task-intent-policy-baseline/v1`, `producer: deterministic-baseline-fallback`, and one admitted `fallback_reason` in candidate provenance.
- Fallback is candidate/IR input only. Both primary and fallback candidates traverse exactly one shared path: structural validation, canonical compilation, serialization, activation, and independent readback before `ResolutionV1` publication; fallback cannot write active state directly.
<!-- /gentle-ai:sdd-init-rubric -->
<!-- /shape:skill -->
