...
`.agent["sdd-init"].prompt` in `opencode.json`: `{file:./prompts/sdd/sdd-init.md}`.
That executable prompt receives the same marker-bounded `skill` contract before
its unique `## Decision Gates` anchor.
...
## Host -> entrypoint and surface map
...
| `opencode` | `~/.config/opencode/skills/sdd-init/SKILL.md`, `~/.config/opencode/skills/sdd-init/references/init-details.md`, `~/.config/opencode/skills/sdd-init/references/rubric-authoring.md` | support/manual surfaces for `sdd-init` | — |
| `opencode` | `~/.config/opencode/prompts/sdd/sdd-init.md` | executable `sdd-init` entrypoint resolved from `.agent["sdd-init"].prompt`; marker-bounded `skill` contract before `## Decision Gates` | — |
